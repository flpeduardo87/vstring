...
export function POS() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'Pix' | 'Dinheiro' | 'Débito' | 'Crédito'>('Pix');
        ...
        <div className="bg-card/50 backdrop-blur-xl p-12 rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center space-y-8 shadow-2xl flex-1 relative overflow-hidden group">
          ...
          {searchTerm && (
            <div className="w-full max-w-xl bg-card border border-white/10 rounded-3xl shadow-2xl divide-y divide-white/5 overflow-hidden z-20 -mt-6 animate-in slide-in-from-top-2 duration-300">
              {inventory
                .filter(p => p.category !== 'Corda' && p.description.toLowerCase().includes(searchTerm.toLowerCase()))
                .slice(0, 6)
                .map(product => (
                  <button
                    key={product.id}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { id: 'Débito', icon: CreditCard, color: 'hover:border-emerald-500/50' },
                  { id: 'Crédito', icon: CreditCard, color: 'hover:border-purple-500/50' }
                ].map((method) => (
                  <Button 
                    key={method.id}
                    type="button" 
