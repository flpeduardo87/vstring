...
export function Dashboard({ setActiveTab }: DashboardProps) {
  ...
  const totalInventoryValue = inventory.reduce((acc, item) => {
    const sellPrice = Number(item.price || 0) * (1 + Number(item.margin || 110) / 100);
    return acc + (sellPrice * Number(item.quantity || 0));
  }, 0);
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row">
          ...
          <Button onClick={() => setActiveTab('services', 'new')} className="bg-primary hover:brightness-110 text-primary-foreground h-12 px-6 rounded-xl font-bold uppercase tracking-widest text-xs transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(217,255,0,0.15)]">
